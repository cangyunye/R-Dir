...
pub fn copy_entries(paths: &[String], dest: &str) -> Result<Vec<String>, String> {
    ...
    for p in paths {
        ...
        copy_recursive(src, &target).map_err(|e| format!("复制 {} 失败：{}", p, e))?;
...
pub fn move_entries(paths: &[String], dest: &str) -> Result<Vec<(String, String)>, String> {
    let dest = Path::new(dest);
    if !dest.is_dir() {
        return Err(format!("目标不是目录：{}", dest.display()));
    }
    ...
    for p in paths {
        ...
        if fs::rename(src, &target).is_err() {
            copy_recursive(src, &target).map_err(|e| format!("移动 {} 失败：{}", p, e))?;
